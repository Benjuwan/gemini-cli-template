...
> [!NOTE]
> - `SKILL`  
> 各種AI（LLM）エージェントに備えることができる特定の作業実施に特化した能力や定型作業フロー手順を指す

- 参考記事：[Claude Code / Codex / Gemini CLI — Skills 機能比較まとめ](https://zenn.dev/hiraoku/articles/e3f750a9fe96dc)

```yaml
---
name: skill-name
description: スキルの説明。いつ使うべきかを明確に書く
---

# スキルの指示内容（Markdown）

ここにエージェントが従うべき手順・ルール・例を記述
```

- `description`の書き方がすべて  
暗黙的な呼び出しは **description フィールドの質** に依存する。
...
| スコープ | パス |
|-----------|------------------------|
| ワークスペース | `.gemini/skills/` |
| ユーザー個人 | `~/.gemini/skills/` |
| 拡張機能（Extension） | Extension にバンドル |

- 優先順位：    
`Workspace > User > Extension`（同名スキルの場合）

### `SKILL.md`呼び出し方法
#### 暗黙的
Gemini がタスクを認識すると `activate_skill` ツールを呼び出し

#### ユーザー承認
アクティベーション時に **確認プロンプト** が表示され、  
名前・目的・アクセスするディレクトリを確認してから有効化（同意型）

#### 特徴的な機能
- 明示的な同意モデル  
  スキル有効化時にUIで確認ダイアログが出る。  
  ディレクトリへのアクセス権限も明示される。

- セッション単位のアクティベーション  
  一度有効化されるとセッション終了まで持続。

### `/skills` コマンド

```bash
/skills list           # 一覧表示
/skills enable <name>  # 有効化
/skills disable <name> # 無効化
--scope workspace      # スコープ指定
```

#### skill-creator 組み込み
「新しいスキルを作って」と頼むだけで雛形を自動生成。
...
#### `GEMINI.md` / `AGENTS.md` の対応
Gemini CLI は `GEMINI.md` と `AGENTS.md` の両方をサポート。  
同ディレクトリに両方ある場合は `GEMINI.md` が優先。

### コンテキストファイル vs スキルの使い分け
